        ...
        <tbody>';
                    ...
                    if ($stmt->rowCount() > 0) {
                        while ($row = $stmt->fetch()) {
                            echo '<form action="supprimerMessage.php" method="post"><tr>
                <td>'.$row["email"].'</td>
                <td>'.$row["text"].'</td>
                <td><input type="hidden" value="'.$row["id"].'" name="id" /><button name="btnS" type="submit" class="btn-reservation" style="font-size:24px"><i class="fas fa-trash-alt"></i></button></td>
            </tr></form>';
                        }
                    }
                    echo '</tbody>
